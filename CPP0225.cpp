...
{   
	testcase()
	{
		...
		ll maxRow = *max_element(hang, hang + n);
		ll maxCol = *max_element(cot, cot + n);
		ll res = max(maxRow, maxCol); // tong  max trong ma tran
		ll ans = 0, i = 0, j = 0;
		while(i < n && j < n)
		{
			ll thieu = min(res - hang[i], res - cot[j]);
			a[i][j] += thieu;
			hang[i] += thieu;
			cot[j] += thieu;
			ans += thieu;
