...
#define memsetfalse(a) memset(a, false, sizeof(a));
...
{
    string id;
    string ten, gioitinh, ns, diachi, mst, ngayhd, day, month, year;
};
...
void nhap(NhanVien &x)
{
    ...
    else if(cnt >= 10) x.id = "000" + to_string(cnt++);
    getline(cin, x.ten);
...
void inds(NhanVien x[], int n)
{
    for(int i = 0; i < n; i++)
    {
        cout << x[i].id << " " << x[i].ten << " " << x[i].gioitinh << " " << x[i].ns << " " << x[i].diachi << " " << x[i].mst << " " << x[i].ngayhd << endl;
    }
}
